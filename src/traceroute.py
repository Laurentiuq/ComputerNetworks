...
def traceroute(dest_addr):
    ...
    for ttl in range(1, max_hops + 1):
        ...
        packet = struct.pack("bbHHh", type, code, Mchecksum, myPID, seq)
...
# if user gives an address, use that, otherwise use google.com
if len(sys.argv) > 1:
    dest = sys.argv[1]
    visited_addresses = traceroute(dest)
    print(visited_addresses)
    plot_map(visited_addresses, color = "purple")

# if user gives a username, use that, otherwise use the current user
if len(sys.argv) > 2:
    username = sys.argv[2]
else:
    username = "laurentiu"
